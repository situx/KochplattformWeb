...
class DBConnection extends  DBConnectionAPI {
  override def selectAllIngredients(language: String): String = ???

  override def insertUser(temp: MainUser): Boolean = ???

  override def selectByName(s: String) = ???

  override def selectUser(name: String) = ???

  def checkUserVote(recipeid: BigInt, id: BigInteger): Boolean = ???

  override def getRating(recipeid: BigInt): String = ???

	override def visitmessage(msg: Message): Message = ???

	def getRecipeUpdates(getUpdaterecipes: util.List[BigInteger]): String = ???

	override def selectRecipeDB(): String = ???

	override def updateRecipe(recipeid: BigInt, recipe: Recipe) = ???

	override def importBackup(b: Boolean, s: String) = ???
  ...
  connection="jdbc:postgresql://localhost:5432/Rezepte?user=postgres&password=post"
	...
	result.next	
	<table width="100%"><tr><th colspan="2">{result.getString(2)}</th></tr>
	...
	<tr><td>Zubereitung:</td><td>{result.getString(7)}</td></tr></table>.toString
	...
	override def register(username:String, password:String)=user.register(username,password)
		...
		def selectraw(ings:scala.collection.mutable.LinkedList[Ingredient])={
		var query="""SELECT DISTINCT id,"recipe".name,category,time,people FROM "recipe","ingredient" WHERE """
		val ing=ings.toArray
		for (i <- ing.indices) {    // anti-idiom array iteration
			query+="\"ingredient\".name='"+ing(i)+"'"
			if(i<ing.length-1)
				query+=" OR "
		}
		println(query)
		<table border="1" cellspacing="0" cellpadding="0" align="center" width="100%"><tr><th>Name</th>
		<th>Kategorie</th><th>Zutaten</th><th>Zubereitungszeit</th><th>Personenanzahl</th><th>Bewertung</th></tr>
		{
		queryEach(query){result => <tr><td>{result.getString(2)}</td><td>{result.getString(3)}</td><td></td><td>{result.getString(4)}</td><td>{result.getString(5)}</td></tr>}
		}
		</table>.toString
	}
