...
class DBCon extends Object with DBConnectionAPI {
  ...
  def getRecipeById(recipeid: String) = {
    <table>
  ...
  override def updatePassword(password: String, userid: BigInt):Boolean = user.updatePassword(password, userid)

  override def updateUser(username: String, password: String, userid: BigInt):Boolean = user.updateUser(username, password, userid)
